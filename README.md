...
## Course packs (imported via Naxos v2 QR code)

Tapping **QR Code** in the options drawer opens a camera scanner (`#scannerOverlay`) that reads a QR code — generated by Naxos v2 and containing a course pack's info — using the [jsQR](https://github.com/cozmo/jsQR) library (loaded from jsDelivr). Once a code is decoded:

- If the QR payload is JSON, its `name` (or `title` / `courseName`) field is used as the course pack's label.
- Otherwise, the raw scanned text is used as the label directly.
- If the camera is unavailable or permission is denied, it falls back to a plain `prompt()` so the pack can still be entered manually — handy for testing without a real Naxos v2 QR code or camera.

Imported packs are stored in `localStorage` (`evia-course-packs`) so they persist across visits, and are added to an in-memory `coursePacks` array (`addCoursePack()` in the script).

**Where they show up:** below Evia, in the same bubble-style pill treatment (white background, grey border) — but as separate tappable pill buttons (`#packWrap`, `.pack-pill`) rather than one text box. They only appear once she's docked in the corner (same ~650ms reveal delay as the speech bubble and the sheet handle), and each has a small `×` to remove it (`renderCoursePacks()`).

Camera access requires HTTPS (or `localhost`) — see the browser permission note under "Installing it as an app" above; the same requirement applies here.
...
**Chat** and **Options** currently just close the sheet on click (wire up real behavior for each inside the `.sheet-row` click handler in the script) — **QR Code** is already wired to `openScanner()`.
...
- Course packs: pill styling in `.pack-pill`; QR payload parsing in `parseCoursePack()`; storage key in `COURSE_PACKS_KEY`; scanner UI in `#scannerOverlay`
